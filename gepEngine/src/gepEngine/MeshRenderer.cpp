...
namespace gepEngine
{
	...
	void MeshRenderer::SetTexture(std::shared_ptr<Textures> _texture)
	{
		m_texture = _texture->GetRendTexture();
		m_renderer.texture(m_texture);
	}

	void MeshRenderer::SetModel(std::shared_ptr<Models> _model)
	{
		m_model = _model->GetRendModel();
	}
